...
train_loader = torch.utils.data.DataLoader(
    datasets.CIFAR10('./MNIST', train=True, download=True,
                   transform=transforms.Compose([
                       transforms.ToTensor(),
                       transforms.Normalize((0.1307,), (0.3081,))
                   ])),
...
test_loader = torch.utils.data.DataLoader(
    datasets.CIFAR10('./MNIST', train=False, download=True,
                   transform=transforms.Compose([
                       transforms.ToTensor(),
                       transforms.Normalize((0.1307,), (0.3081,))
                   ])),
...
class DeformNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.conv4(x, offsets)) # pass the generated offsets to conv4 which is the DeformConv2D process - 2nd step in DCN
        x = self.bn4(x)
...
model.apply(init_weights) # initialize weights for the each layers in the DeformNet model
model.offsets.apply(init_conv_offset) #initialize weight (all zeros) to the offsets in the DCN
